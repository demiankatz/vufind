...
class MultiBackendTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Methods to mock in mock ILS drivers, in method name => return value format.
     *
     * @var array
     */
    protected $mockIlsMethods = [
        'cancelHolds' => [],
        'cancelILLRequests' => [],
        'cancelStorageRetrievalRequests' => [],
        'checkRequestIsValid' => null,
        'checkILLRequestIsValid' => null,
        'checkStorageRetrievalRequestIsValid' => null,
        'getCancelHoldDetails' => '',
        'getCancelILLRequestDetails' => '',
        'getCancelStorageRetrievalRequestDetails' => '',
        'getConfig' => [],
        'getDefaultPickUpLocation' => '',
        'getDefaultRequestGroup' => false,
        'getMyILLRequests' => null,
        'getILLPickUpLibraries' => null,
        'getILLPickUpLocations' => null,
        'getPickUpLocations' => [],
        'getRenewDetails' => '',
        'getRequestGroups' => [],
        'loginIsHidden' => false,
        'placeHold' => null,
        'placeILLRequest' => null,
        'placeStorageRetrievalRequest' => null,
        'renewMyItems' => [],
        'getAccountBlocks' => false,
        'getRequestBlocks' => false,
    ];
    ...
    public function testILSConfigurationPathWithoutDriverConfigPath()
    {
        ...
        $ils = $this->getMockILS(Voyager::class);
        $driver = new MultiBackend(
            $mockPM,
            $this->getMockILSAuthenticator(),
    ...
    public function testGetDriver()
    {
        ...
        $ILS = $this->getMockILS(Voyager::class, ['init', 'setConfig']);
    ...
    {
        $driver = $this->getDriver();
        $voyager = $this->getMockILS(Voyager::class, ['init']);
        ...
        $dummy = $this->getMockILS(Voyager::class, ['init', 'supportsMethod']);
    ...
    {
        $ils1 = $this->getMockILS(Voyager::class, ['init', 'getHolding']);
        ...
        $ils2 = $this->getMockILS(Demo::class, ['init', 'getHolding']);
        ...
        $driver = $this->getMultibackendForDrivers(['d1' => $ils1, 'd2' => $ils2]);
    ...
    public function testGetPurchaseHistory()
    {
        ...
        $ILS = $this->getMockILS(Voyager::class, ['init', 'getPurchaseHistory']);
        $ILS->expects($this->once())
    ...
    {
        $ILS = $this->getMockILS(Voyager::class, ['init', 'getStatus']);
    ...
    {
        $ils1 = $this->getMockILS(Voyager::class, ['init', 'getStatuses']);
        $ils1->expects($this->exactly(2))
            ->method('getStatuses')
            ->with(
        ...
        $ils2 = $this->getMockILS(Unicorn::class, ['init', 'setConfig', 'getStatuses']);
        ...
        $ils3 = $this->getMockILS(Demo::class, ['init', 'setConfig', 'getStatuses']);
    ...
    public function testDefaultDriver()
    {
        ...
        $ILS = $this->getMockILS(Voyager::class, ['getMyTransactions', 'init']);
        ...
        $ILS = $this->getMockILS(Unicorn::class, ['getMyTransactions', 'init']);
        $ILS->expects($this->atLeastOnce())
            ->method('getMyTransactions')
            ->with($patron)
            ->will($this->returnValue(true));
    ...
    public function testGetNewItems()
    {
        ...
        $ILS = $this->getMockILS(Voyager::class, ['getNewItems', 'init']);
        $ILS->expects($this->once())
            ->method('getNewItems')
    ...
    public function testFindReserves()
    {
        ...
        $ILS = $this->getMockILS(Voyager::class, ['findReserves', 'init']);
        $ILS->expects($this->once())
            ->method('findReserves')
    ...
    public function testPatronLogin()
    {
        ...
        $ILS = $this->getMockILS(Voyager::class, ['patronLogin']);
        $ILS->expects($this->once())
            ->method('patronLogin')
    ...
    {
        $ils1 = $this->getMockILS(Voyager::class, ['init', 'checkRequestIsValid']);
        ...
        $ils2 = $this->getMockILS(Demo::class, ['init', 'checkRequestIsValid']);
        $ils2->expects($this->once())
            ->method('checkRequestIsValid')
            ->with('bibid', ['id' => 'itemid'], $this->getPatron('username'))
            ->will(
        ...
        $demo = $this->getMockILS(Demo::class, ['init', 'checkRequestIsValid']);
        ...
        $driver = $this->getMultibackendForDrivers(['d2' => $demo]);
        $result = $driver->checkRequestIsValid(
            'd1.bibid',
            [
                'id' => 'd1.itemid'
    ...
    {
        //Set up the mock driver to be retrieved
        $ILS = $this->getMockILS(Voyager::class, ['setConfig', 'init']);
        $ILS->expects($this->once())
            ->method('setConfig')
            ->with(['config' => 'values']);
        $ILS->expects($this->once())
    ...
    protected function getMultibackendForDrivers(
        ...
        $driverPrefixMap = [];
        $i = 0;
        foreach ($drivers as $prefix => $driver) {
            $i++;
            $driverName = "Driver$i";
            $driverMap[$driverName] = $driver;
            $driverPrefixMap[$prefix] = $driverName;
        }
        $sm = $this->getMockBuilder(\VuFind\ILS\Driver\PluginManager::class)
            ->disableOriginalConstructor()->getMock();
        ...
        $driver = $this->getDriver($sm);
        $this->setProperty($driver, 'drivers', $driverPrefixMap);
    ...
    /**
     * Get constructor arguments for a mock ILS driver.
     *
     * @param string Class we are mocking
     *
     * @return array
     */
    protected function getMockIlsDriverConstructorArgs($class)
    {
        if ($class === Demo::class) {
        ...
        return $class === Sample::class ? [] : [new \VuFind\Date\Converter()];
    }
     ...
     * @param array  $methods Array of methods to stub for calling code's expectations
     *
     * @return \VuFind\ILS\Driver\AbstractBase
     */
    protected function getMockILS($type, $methods = [])
    {
        $mock = null;
        $constructorArgs = $this->getMockIlsDriverConstructorArgs($type);
        $allMethods = array_unique(
            array_merge(array_keys($this->mockIlsMethods), $methods)
        );
        $onlyMethods = array_filter(
            $allMethods,
            function ($method) use ($type) {
                return method_exists($type, $method);
            }
        );
        $addMethods = array_diff($allMethods, $onlyMethods);
        $mock = $this->getMockBuilder($type)
            ->onlyMethods($onlyMethods)
            ->addMethods($addMethods)
            ->setConstructorArgs($constructorArgs)
            ->getMock();
        foreach ($this->mockIlsMethods as $method => $retVal) {
            if (!in_array($method, $methods)) {
                $mock->expects($this->any())->method($method)
                    ->will($this->returnValue($retVal));
            }
        }
        if (in_array('supportsMethod', $allMethods)) {
            $mock->expects($this->any())->method('supportsMethod')
                ->will($this->returnValue(false));
        }
